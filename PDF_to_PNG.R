...
# Convert for Chapter 13 ----
# PDF directory
pdf_dir <- "../alldiagrams/"

# Chapter 13 figures
ch13_base <- list.files(pdf_dir, pattern = glob2rx("fig13.*"))

# Chapter 13 PDF paths
ch13_pdf <- paste0(pdf_dir, ch13_base)

# PNG output
ch13_png <- paste0("lectures/fig/", gsub(".pdf", ".png", ch13_base))

# Create commands
ch13_cmds <- paste0("magick -density 300 -quality 95 ",
                    ch13_pdf, " ",
                    ch13_png)

# Execute
lapply(ch13_cmds, function(x) shell(x))

# Convert for Chapter 14 ----
# PDF directory
pdf_dir <- "../alldiagrams/"

# Chapter 14 figures
ch14_base <- list.files(pdf_dir, pattern = glob2rx("fig14.*"))

# Chapter 14 PDF paths
ch14_pdf <- paste0(pdf_dir, ch14_base)

# PNG output
ch14_png <- paste0("lectures/fig/", gsub(".pdf", ".png", ch14_base))

# Create commands
ch14_cmds <- paste0("magick -density 300 -quality 95 ",
                    ch14_pdf, " ",
                    ch14_png)

# Execute
lapply(ch14_cmds, function(x) shell(x))


# Convert for Chapter 9 ----
# PDF directory
pdf_dir <- "../alldiagrams/"

# Chapter 9 figures
ch9_base <- list.files(pdf_dir, pattern = glob2rx("fig9.*"))

# Chapter 9 PDF paths
ch9_pdf <- paste0(pdf_dir, ch9_base)

# PNG output
ch9_png <- paste0("lectures/fig/", gsub(".pdf", ".png", ch9_base))

# Create commands
ch9_cmds <- paste0("magick -density 300 -quality 95 ",
                    ch9_pdf, " ",
                    ch9_png)

# Execute
lapply(ch9_cmds, function(x) shell(x))
